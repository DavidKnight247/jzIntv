...
mkdir("/media/data/local/home/.jzintellivision",S_IRWXU);
mkdir("/media/data/local/home/.jzintellivision/bios",S_IRWXU);
mkdir("/media/data/local/home/.jzintellivision/overlays",S_IRWXU);
mkdir("/media/data/local/home/.jzintellivision/configfiles",S_IRWXU);
#endif
...
jzp_printf("cpu.now = %-8d  stic.now = %-8d diff = %-8d step = %-8d\n", (int)intv.cp1600.periph.now, (int)intv.stic.stic_cr.now, (int)intv.cp1600.periph.now-(int)intv.stic.stic_cr.now, (int)max_step);
	...
	if (!foundconfig)
	{
		...
		foundconfig=1;
		FILE *configuration;
		configuration=fopen("/media/data/local/home/.jzintellivision/.configuration","r");
		if (!configuration) {
...
//now save new configuration
			FILE *configuration;
			configuration=fopen("/media/data/local/home/.jzintellivision/.configuration","w");
